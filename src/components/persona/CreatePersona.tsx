...
const personaSchema = z.object({
    ...
    bodyColor: z.string().regex(/^#[0-9A-F]{6}$/i, { message: "Please enter a valid hex color code." }),
...
const categories = ["Therapist", "Friend", "Tutor", "Coach", "Mentor", "Companion", "Expert", "Assistant", "Entertainer", "Other"]
...
const defaultColors = ["#4285f4", "#ea4335", "#fbbc04", "#34a853", "#fa7b17", "#f538a0", "#a142f4", "#24c1e0", "#ff6b81", "#ff8a65", "#ffb74d", "#81c784", "#64b5f6", "#9575cd"]
...
export default function CreatePersona() {
    ...
    const { createPersona, loading, error } = usePersona()

    const form = useForm<PersonaFormValues>({
        resolver: zodResolver(personaSchema),
        defaultValues: {
            name: "", category: "Friend", description: "", userPrompt: "", tags: [], bodyColor: defaultColors[0],
            gender: "", language: "", age: 25, isPublic: true, personality: "", avatarUrl: "",
        },
    })
    ...
    const handleEnhanceWithAI = async () => {
        ...
        try {
            ...
            if (!form.getValues("age") || form.getValues("age") === 25) form.setValue("age", enhanced.age)
            if (!form.getValues("gender")) form.setValue("gender", enhanced.gender)
            if (!form.getValues("category") || form.getValues("category") === "Friend") form.setValue("category", enhanced.category)
            if (!form.getValues("language")) form.setValue("language", enhanced.language)
            
            form.setValue("personality", enhanced.personality)
            form.setValue("userPrompt", enhanced.enhancedDescription)
            if (form.getValues("tags").length === 0) form.setValue("tags", enhanced.tags)

            setShowEnhanced(true)
            setIsAdvancedOpen(true) // Auto-expand to show AI-filled fields
            toast.success("✨ AI enhanced your persona!")
        } catch (error) {
            toast.error("Failed to enhance persona with AI")
        } finally {
    ...
    const handleNavigate = async (personaId: string, name: string, avatarUrl: string, bodyColor: string) => {
        if (!user) {
        ...
        try {
            ...
            if (!roomId) {
                roomId = await createRoom({ userId: user.uid, personaId, title: name, avatarUrl, bodyColor });
    ...
    const onSubmit = async (data: PersonaFormValues) => {
        try {
            ...
            if (personaId) {
                ...
                handleNavigate(personaId, data.name, data.avatarUrl ?? "", data.bodyColor)
            }
        } catch (err) {
                        ...
                        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 w-full max-w-2xl">
                            ...
                            <FormField control={form.control} name="name" render={({ field }) => (
                                <FormItem>
                                    ...
                                    <FormControl>
                                        <Input placeholder="e.g., Dr. Sarah" {...field} className="border-none bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10" />
                                    </FormControl>
                                    <FormMessage />
                                </FormItem>
                            )} />
                            ...
                            <FormField control={form.control} name="description" render={({ field }) => (
                                <FormItem>
                                    ...
                                    <FormControl>
                                        <Textarea placeholder="Describe the persona briefly..." rows={3} {...field} className="border-none bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10" />
                                    </FormControl>
                                    <FormMessage />
                                </FormItem>
                            )} />
                            ...
                            {!showEnhanced && (
                                <Button type="button" onClick={handleEnhanceWithAI} disabled={isEnhancing} variant="outline"
                                    ...
                                    {isEnhancing ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />AI is enhancing...</> : <>✨ Generate Details with AI</>}
                                </Button>
                            )}
                            ...
                            <Collapsible open={isAdvancedOpen} onOpenChange={setIsAdvancedOpen} className="w-full space-y-4">
                                <CollapsibleTrigger asChild>
                                    <Button type="button" variant="ghost" className="w-full justify-between p-4 hover:bg-black/5 dark:hover:bg-white/5">
                                ...
                                <CollapsibleContent className="space-y-6">
                                    <div className="grid grid-cols-2 gap-4">
                                        <FormField control={form.control} name="age" render={({ field }) => (
                                            <FormItem>
                                                ...
                                                <FormControl>
                                                    <Input type="number" min="1" max="120" {...field} onChange={(e) => field.onChange(parseInt(e.target.value) || 0)}
                                        ...
                                        <FormField control={form.control} name="gender" render={({ field }) => (
                                            <FormItem>
                                                ...
                                                <Select onValueChange={field.onChange} value={field.value}>
                                                    ...
                                                    <SelectContent>
                                                        {genders.map(g => <SelectItem key={g} value={g}>{g.toUpperCase()}</SelectItem>)}
                                                    </SelectContent>
                                                </Select>
                                                <FormMessage />
                                            </FormItem>
                                    ...
                                    <FormField control={form.control} name="userPrompt" render={({ field }) => (
                                        <FormItem>
                                            ...
                                            <FormControl>
                                                <Textarea rows={4} {...field} className="border-none bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10" />
                                            </FormControl>
                                            <FormMessage />
                                        </FormItem>
                                    )} />
                                        ...
                                        <FormItem>
                                            ...
                                            <div className="space-y-3">
                                                <div className="flex items-center gap-3">
                                                    <div className="h-12 w-12 rounded-full" style={{ backgroundColor: field.value }}>
                                                ...
                                                <div className="flex flex-wrap gap-2">
                                                    {defaultColors.map((color) => (
                                                        <button key={color} type="button" className={`w-8 h-8 rounded-md border-2 ${field.value === color ? 'border-primary' : 'border-border'}`}
                                                            style={{ backgroundColor: color }} onClick={() => field.onChange(color)} />
                                                    ))}
                                                </div>
                                            </div>
                                        </FormItem>
                            ...
                            <Button type="submit" className="w-full" disabled={loading}>
                                {loading ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Creating...</> : "Create Persona"}
                            </Button>
                        </form>
                    </Form>
                </CardContent>
