...
const ShuffleHero = ({ handleEarlyAccess }: { handleEarlyAccess?: MouseEventHandler<HTMLButtonElement> }) => {
    return (
        <section className="w-full px-8 py-12 grid grid-cols-1 md:grid-cols-2 items-center gap-8 max-w-7xl mx-auto">
            <div>
                <span className="block mb-4 text-xs md:text-sm text-primary font-medium text-white">
                    {EN_FEATURE_THREE.badge}
                </span>
                <h3 className="text-3xl md:text-4xl lg:text-5xl font-bold text-white">
                    {EN_FEATURE_THREE.title}
                </h3>
                <p className="text-base md:text-lg text-white/70 my-4 md:my-6">
    ...
    if (squares.length === 0) {
        return (
            <div className="grid grid-cols-3 grid-rows-3 h-[450px] gap-1">
                {[...Array(9)].map((_, i) => (
                    <div key={i} className="w-full h-full rounded-md bg-white/5 animate-pulse"></div>
                ))}
            </div>
        );
    }
